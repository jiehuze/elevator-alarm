...
//            "</if> " +
    ...
    @Select({
            ...
            "  SUM(CASE WHEN brand_counts.brand_count <= 10 THEN brand_counts.brand_count ELSE 0 END) AS smallBrandsCount, ",
            "  SUM(brand_counts.brand_count) AS totalBrandsAll, ",
            "  CASE WHEN SUM(brand_counts.brand_count) > 0 THEN ",
            "    ROUND(SUM(CASE WHEN brand_counts.brand_count <= 10 THEN brand_counts.brand_count ELSE 0 END) * 100.0 / SUM(brand_counts.brand_count), 2) ",
            "  ELSE 0 END AS smallBrandPercentage ",
            "]]>",
            "FROM (",
